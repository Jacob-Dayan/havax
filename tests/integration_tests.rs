...
"#;
    ...
    fn test_treesitter_custom_struct_enum_and_method_ast_completions() {
        ...
        editor.trigger_completion();
        assert!(editor.completion.visible, "Completion menu must be visible on `user.`");
        let method_labels: Vec<&str> = editor.completion.items.iter().map(|it| it.label.as_str()).collect();
        assert!(method_labels.contains(&"get_username"), "Must contain get_username method from AST impl");
        assert!(method_labels.contains(&"deactivate"), "Must contain deactivate method from AST impl");
        assert!(method_labels.contains(&"authenticate"), "Must contain authenticate from trait impl");
        assert!(method_labels.contains(&"logout"), "Must contain logout from trait impl");
        ...
        editor.trigger_completion();
        assert!(editor.completion.visible);
        let struct_labels: Vec<&str> = editor.completion.items.iter().map(|it| it.label.as_str()).collect();
        assert!(struct_labels.contains(&"new"), "Must contain associated function new");
        assert!(struct_labels.contains(&"get_username"), "Must contain get_username");
        assert!(struct_labels.contains(&"deactivate"), "Must contain deactivate");
        ...
        assert!(editor.completion.visible);
        let enum_labels: Vec<&str> = editor.completion.items.iter().map(|it| it.label.as_str()).collect();
        assert!(enum_labels.contains(&"Active"), "Must contain enum variant Active");
        assert!(enum_labels.contains(&"Suspended"), "Must contain enum variant Suspended");
        assert!(enum_labels.contains(&"Deleted"), "Must contain enum variant Deleted");
    }
